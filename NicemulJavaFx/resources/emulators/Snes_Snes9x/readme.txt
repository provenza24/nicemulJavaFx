...
Snes9x.exe, readme.txt, license.txt fmodex.dll changes.txt

v1.52  January 12th, 2009
=================
...
The minimum os version for this port is Windows 2000
DirectX 9.0c (when using Direct3D/XAudio2)
DirectX 6.1b or later (when using DirectDraw/DirectSound)
133mhz processor BARE MINIMUM (1ghz+ rec for best settings)
16MB ram BARE MINIMUM (128megs+ rec)
...
un-zipped the snes9x.exe and the fmodex.dll files and double-click on
...
stored your SNES games. Single-click and then press Open to load and start the
game.
...
- If all sound menu options are grayed out, or an error dialog about Snes9x not
  being able to initialize sound output is displayed: Make sure at least
  DirectX 6 (for DirectSound output) or DirectX 9 (for XAudio2 output) is
  installed. If you want to use any of the FMOD output methods make sure fmodex.dll
  is in the same directory as snes9x.exe
...
- You've clicked the Mute button in the
...
- A continuous, crackling sound or buzz can be heard.
  Try increasing the sound buffer size and/or activate sound sync. The default
  buffer sizes (automatically set when changing the sound output method) are
  usually a good value. You can also try lowering the sound input rate.

  You can also try one of the FMOD output methods. 

- Sound is lagged/delayed compared to on-screen action. Your sound buffer setting
  might be too large - if you can, reduce it.
- Sound quality is poor on all games. You might have a noisy sound card
  (usually cheaper sound cards), turning on 16-bit, sync-sound and/or increasing
  the playback rate might help.
...
Lowering the sound playback rate or selecting 8-bit mono sound will also help
